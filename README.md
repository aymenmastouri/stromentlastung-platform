...
Jeder Dienst öffnet bei laufendem Betrieb seine H2-Konsole unter dem Kontextpfad, etwa `http://localhost:8094/api/zahlungen/h2-console` (entsprechend `8091/api/unternehmen`, `8092/api/antraege`, `8093/api/bescheide`). Benutzer `sa`, kein Passwort; die JDBC-URL lautet im Entwicklungsbetrieb `jdbc:h2:file:./data/zahlung;AUTO_SERVER=TRUE` und im Containerbetrieb `jdbc:h2:file:/app/data/zahlung;AUTO_SERVER=TRUE`. Im Container ist der Zugriff vom Rechner aus für H2 ein entfernter und wird über `STROMENTLASTUNG_H2_CONSOLE_REMOTE` freigegeben; die Ports hören dort nur auf 127.0.0.1. Dort lässt sich der Schutz des Vorgangsprotokolls zeigen: ein `UPDATE` oder `DELETE` auf `vorgangsereignis` im Vorgangsdienst lehnt der Trigger ab. Wer lieber ein Werkzeug wie IntelliJ oder DBeaver nutzt, verbindet sich mit derselben URL über den absoluten Pfad der Datei; `AUTO_SERVER` erlaubt den Parallelzugriff bei laufendem Dienst.
