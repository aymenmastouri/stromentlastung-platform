...
# Bring the branch into the repository and report the commit, or nothing when it does not exist.
fetch_branch() {
  local repo=$1 branch=$2 dir="$ROOT/stromentlastung-$repo"
  git -C "$dir" fetch -q origin "+refs/heads/$branch:refs/remotes/origin/$branch" 2>/dev/null && {
    git -C "$dir" rev-parse "origin/$branch"; return 0; }
  if [ -d "$CLONE/$repo/.git" ]; then
    git -C "$dir" fetch -q "$CLONE/$repo" "+refs/heads/$branch:refs/remotes/pipeline/$branch" 2>/dev/null && {
      git -C "$dir" rev-parse "pipeline/$branch"; return 0; }
  fi
  return 1
}
